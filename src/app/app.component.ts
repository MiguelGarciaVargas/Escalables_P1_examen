...
export class AppComponent {
    ...
    public isSortingAscending = true;
    ...
    // public openEditWindow(show: Show): void {
    //     // this.editingShow = { ...show };
    //     // const modalRef = this.modalService.open('editModal'); // Abre el modal
    //     // const modalElement = this.exampleModal.nativeElement;

    //     const modalElement = new Modal(this.exampleModal.nativeElement);
    ...
    public openEditWindow(show: Show): void {
        ...
        if (this.exampleModal) {
            this.editingName = show.name;
            const modalElement = new Modal(this.exampleModal.nativeElement);
            modalElement.show();
        }
    }

    public editCard(show: Show) {
        const index = this.shows.findIndex(s => s.name === this.editingName);
        
        this.shows[index].name = show.name;
        this.shows[index].description = show.description;
        this.shows[index].image = show.image;

        console.log(this.shows[index].name);
    ...
    public orderByYear(): void {
        this.isSortingAscending ? this.shows.sort((a, b) => a.year - b.year) : this.shows.sort((a, b) => b.year - a.year);
    }

    public orderByEpisodeNumber(): void {
        this.isSortingAscending ? this.shows.sort((a, b) => a.episodes - b.episodes) : this.shows.sort((a, b) => b.episodes - a.episodes);
    }

    public changeSortDirection(isAcending: boolean): void {
        this.isSortingAscending = isAcending;
    }

    public orderByName(): void {
        this.isSortingAscending
            ? this.shows.sort((a, b) => a.name.localeCompare(b.name))
            : this.shows.sort((a, b) => b.name.localeCompare(a.name));
    }
